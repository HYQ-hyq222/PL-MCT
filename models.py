...
# ------------------------------------------------------------------------------
from lib.models.connect import AdjustLayer, box_tower_reg, Process_conf
...
from lib.utils.image_utils import corner2center2
...
class USOT_(nn.Module):
    ...
    def pred_offset_to_image_bbox2(self, bbox_pred, batch):
        """
        Convert bbox from the predicted response map axis to the image-level axis
        """
    ...
    def prpool_feature(self, features, bboxs):   # features(1,256,31,31) bbox(1,4)
        """
        ...
        return prroi_pool(features, bboxs_index, (7, 7), 1.0)
    ...
    def track(self, x, template_mem=None, score_mem=None):
        ...
        if template_mem is not None:
            ...
            # 1,256,31,31  1,256,7,7
            # 7,256,7,7  1,7
            bbox_pred, cls_pred, cls_feature, reg_feature, cls_memory_pred, f_, search_ = self.connect_model(xf, kernel=self.zf,
                                                                                                memory_kernel=template_mem,
                                                                                                memory_confidence=score_mem)
            conf = self.process_conf(f_, search_)

            # Here xf is the feature of search areas which will be cropped soon according to the final bbox
            return cls_pred, bbox_pred, cls_memory_pred, xf, conf
        else:
            # Track with offline module only
            bbox_pred, cls_pred, _, _, _, f_, search_ = self.connect_model(xf, kernel=self.zf)
            conf = self.process_conf(f_, search_)

            return cls_pred, bbox_pred, None, None, conf

    def extract_memory_feature(self, ori_x=None, xf=None, search_bbox=None):  # ori_x(1,3,255,255)  # bbox(1,4)
        # Note that here search bbox is the bbox on the deep feature (not on the original search frame)
        if ori_x is not None:
            _, xf = self.feature_extractor(ori_x)    # xf(1,1024,31,31)
            xf = self.neck(xf, crop=False)           # xf(1,256,31,31)
        features = self.prpool_feature(xf, search_bbox)   # features(1,256,7,7)
        return features

    def forward(self, template, search, label=None, reg_target=None,
                reg_weight=None, template_bbox=None, search_memory=None,
                search_bbox=None, cls_ratio=0.40, pos_list=None, type_list=None, type_settings=None, do_conf=False):
        """
        ...
        if not do_conf:
            ...
            bbox_pred, cls_pred, _, _, _, f_, search_ = self.connect_model(xf, kernel=zf)
            # get loss
            cls_loss = self._weighted_BCE(cls_pred, label)
            reg_loss = self.add_iouloss(bbox_pred, reg_target, reg_weight)

            return cls_loss, None, reg_loss, None

        elif search_memory is not None:
            # Original Siamese fg/bg cls and bbox reg branch (self-track in paper)
            bbox_pred, cls_pred, cls_x, _, _, f_, search_ = self.connect_model(xf, kernel=zf)
            ...
            # 带权重的conf损失
            conf = self.process_conf(f_, search_)
            pos_list = pos_list.view(-1)
            batch = pos_list.shape[0]
            weights = []
            for i in range(batch):
                type_key = str(type_list[i].item())
                weights.append(type_settings[type_key])
            weights = torch.tensor(weights)
            weights = weights.to(type_list.device)
            conf_loss = nn.BCELoss(reduction='none')(conf, pos_list)
            conf_loss_weighted = (conf_loss * weights).sum() / weights.sum()


            # Now begin to calculate cycle memory loss
            # Extract deep features for memory search areas
            batch, mem_size, cx, hx, wx = search_memory.shape   # 8,2,3,511,511
            search_memory = search_memory.view(-1, cx, hx, wx)   # 16,3,255,255
            _, xf_mem = self.feature_extractor(search_memory)
            xf_mem = self.neck(xf_mem, crop=False) #13,256,31,31

            # Prepare feature for mem_forward_cls (forward tracking with the online module)
            search_pooled_feature = self.prpool_feature(xf, search_bbox)    # 8,256,7,7
            batch, cspf, wspf, hspf = search_pooled_feature.shape
            search_pooled_feature = search_pooled_feature.view(batch, 1, cspf, wspf, hspf) # 8,1,1256,7,7
            ...
            off_forward_bbox, off_forward_cls, forward_x_store, _, _, _, _ = self.connect_model(xf_mem, kernel=zf_mem)

            # Get the mem_forward_cls score in memory search areas (tracking with online module)
            fake_confidence = torch.ones(batch * mem_size, 1)  # 16,1
            _, _, _, _, mem_forward_cls, _, _ = self.connect_model(xf_mem, memory_kernel=search_pooled_feature,
                                                             memory_confidence=fake_confidence,
                                                             cls_x_store=forward_x_store)  # 16,1,25,25
            mem_forward_cls = mem_forward_cls.view(batch, mem_size, -1)   # 8,2,625
            off_forward_cls = off_forward_cls.view(batch, mem_size, -1)   # 8,2,625
            ...
            forward_res_map = cls_ratio * off_forward_cls + (1 - cls_ratio) * mem_forward_cls  # 8,2,625
            best_forward_cls = forward_res_map.max(dim=2)
            best_forward_cls_argmax = best_forward_cls.indices.view(batch, mem_size, 1, 1)
            best_forward_cls_argmax = best_forward_cls_argmax.repeat(1, 1, 1, 4)    # 8,2,1,1
            bbox_pred_to_img = self.pred_offset_to_image_bbox(off_forward_bbox, batch)   # 16,4,25,25
            bbox_pred_to_img = bbox_pred_to_img.view(batch, mem_size, 4, -1).transpose(2, 3)  # 8,2,625,4
            best_mem_bbox = torch.gather(bbox_pred_to_img, dim=2,   # 16,4
                                         index=best_forward_cls_argmax).view(batch * mem_size, 4)
            best_forward_cls_score = best_forward_cls.values.detach()         # 8,2
            best_forward_bbox_pool = self.image_bbox_to_prpool_bbox(best_mem_bbox).detach()   # 16,4

            # PrPool intermediate features from memory search areas as the memory queue
            pooled_mem_features = self.prpool_feature(xf_mem, best_forward_bbox_pool)   # 16,256,7,7
            # Backward track from memory queue to the search area in the template frame
            _, _, _, _, backward_res_map, _, _ = self.connect_model(xf, memory_kernel=pooled_mem_features,
            ...
            return cls_loss_ori, cls_memory_loss, reg_loss, conf_loss_weighted
            ...
            reg_loss = self.add_iouloss(bbox_pred, reg_target, reg_weight)
            # # # 不带权重的conf损失
            # # conf_loss_weighted = self.conf_loss_func(conf, pos_list)
            # 带权重的conf损失
            batch = pos_list.shape[0]
            weights = []
            for i in range(batch):
            ...
            conf_loss_weighted = (conf_loss * weights).sum() / weights.sum()
            # 输出conf损失
            return cls_loss, None, reg_loss, conf_loss_weighted
...
class USOT(USOT_):
    def __init__(self, settings=None):
        ...
        self.process_conf = Process_conf(in_channels=256)
