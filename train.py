...
from lib.dataset_loader.datasets_SSL import USOTDataset
from lib.config.config_usot import config, update_config
from lib.utils.usot_eval import USOT_EVAL
import torch, gc
...
def parse_args():
    ...
    parser.add_argument('--cfg', type=str,
                        default='/home/zcy/code/hyqcode/autodlusot/USOT-main/experiments/train/SSL/USOT.yaml', help='yaml configure file name')
...
def build_opt_lr(cfg, model, current_epoch=1):
    ...
    if cfg.USOT.TRAIN.CONF_EPOCH <= current_epoch < cfg.USOT.TRAIN.MEMORY_EPOCH:  # 11-20 正确的  11<= epoch < 21
        for param in model.process_conf.parameters():
            param.requires_grad = True
        trainable_params += [{'params': model.process_conf.parameters(),
                          'lr': cfg.USOT.TRAIN.BASE_LR}]
    elif current_epoch >= cfg.USOT.TRAIN.MEMORY_EPOCH:  # 21-40
        for param in model.neck.parameters():
            param.requires_grad = True
        for param in model.connect_model.parameters():
            param.requires_grad = True
...
def usot_train(train_loader, model, optimizer, epoch,
    ...
    for iter, input in enumerate(train_loader):
        ...
        template = input[0].to(device)  # tensor 16,3,255,255
        ...
        pos_list = input[6].float().to(device)
        type_list = input[7].to(device)

        if len(input) >= 9:
            # Cycle memory training
            search_memory = input[8].to(device)    #16,3,3,255,255
            search_bbox = input[9].to(device)
        else:
            search_memory = None
            search_bbox = None
        if epoch >= config.USOT.TRAIN.CONF_EPOCH:
            do_conf = True # 都训练conf
        else:
            do_conf = False   # 只单帧
        ...
        # Model forward logic
        cls_loss_ori, cls_loss_memory, reg_loss, conf_loss = model(template, search, label_cls,
                                                          ...
                                                          pos_list=pos_list,
                                                          type_list=type_list,
                                                          type_settings=cfg.USOT.TRAIN.TYPE_SETTINGS, do_conf=do_conf)
            ...
            for i_ep in range(len(lambda_shift_epochs) - 1):
                if lambda_shift_epochs[i_ep] <= epoch <= lambda_shift_epochs[i_ep + 1]:
                    loss = lambda1_list[i_ep] * cls_loss_ori + \
                           (lambda_total - lambda1_list[i_ep]) * cls_loss_memory + 1.0 * reg_loss + cfg.USOT.TRAIN.LAMBDA_CONF2 * conf_loss
                    done = True
                    break
            if not done:
                # From last epoch in lambda1_list to the final epoch
                lambda_1 = lambda1_list[-1]
                loss = lambda_1 * cls_loss_ori + (lambda_total - lambda_1) * cls_loss_memory + 1.0 * reg_loss + cfg.USOT.TRAIN.LAMBDA_CONF2 * conf_loss
        else:
            if epoch >= config.USOT.TRAIN.CONF_EPOCH:
        ...
        try:
            ...
            conf_loss = 0
        conf_losses.update(conf_loss, template.size(0))
        ...
        if (iter + 1) % cfg.PRINT_FREQ == 0:
            logger.info(
                ...
                '\t CLS_ORI_Loss:{cls_loss_ori.avg:.5f} \t CLS_MEMORY_Loss:{cls_loss_memory.avg:.5f} \t REG_Loss:{reg_loss.avg:.5f} '
                '\t CONF_Loss:{conf_loss.avg:.5f} \t Loss:{loss.avg:.5f}'.format(
                    epoch, iter + 1, len(train_loader), lr=cur_lr, batch_time=batch_time, data_time=data_time,
                    loss=losses, cls_loss_ori=cls_losses_ori, cls_loss_memory=cls_losses_memory, reg_loss=reg_losses, conf_loss=conf_losses))
...
def main():
    ...
    model = load_pretrain(model, config.USOT.TRAIN.PRETRAIN)

    # 数据增强的yaml文件
    with open(os.path.join(config.AUG_SETTINGS), 'r', encoding='utf-8') as f:  ##打开参数文件
        cond = f.read()  # 读取数据路径值
        AUG_SETTINGS = yaml.load(cond, Loader=yaml.FullLoader)  # 下载文件数据
        crop_settings = AUG_SETTINGS['CROP_SETTINGS']
    ...
    for epoch in range(config.USOT.TRAIN.START_EPOCH, config.USOT.TRAIN.END_EPOCH + 1):
        ...
        if epoch >= config.USOT.TRAIN.MEMORY_EPOCH:
            aug_settings = AUG_SETTINGS['AUG_SETTINGS_3']
            checkpoint_pth = os.path.join(config.CHECKPOINT_DIR, 'checkpoint_e%d.pth' % (epoch - 1))
            ...
            if epoch >= config.USOT.TRAIN.MEMORY_EPOCH:
                ...
                with torch.no_grad():
                    json_path_dict = usot_eval.forward(config, checkpoint_pth, epoch, i, j)
                    i += 1
                    j += 1
             # 多帧阶段出现问题的时候用的
             #    json_path_dict = {'VID':"/home/zcy/code/hyqcode/autodlusot/USOT-main/scripts/var_vid/var109/eval/VID/eval_e21.json",
             # 'GOT10K':"/home/zcy/code/hyqcode/autodlusot/USOT-main/scripts/var_vid/var109/eval/GOT10K/eval_e21.json"}

            train_set = USOTDataset(config, aug_settings, crop_settings, epoch, json_path_dict)
            train_set.cycle_memory = True
        elif epoch >= config.USOT.TRAIN.CONF_EPOCH:
            aug_settings = AUG_SETTINGS['AUG_SETTINGS_2']
            json_path = ''
            train_set = USOTDataset(config, aug_settings, crop_settings, epoch, json_path)
            train_set.cycle_memory = False
        else:
            aug_settings = AUG_SETTINGS['AUG_SETTINGS_1']                                                                                                                                                                                     
            json_path = ''
            train_set = USOTDataset(config, aug_settings, crop_settings, epoch, json_path)
            train_set.cycle_memory = False
        ...
        if epoch == config.USOT.TRAIN.UNFIX_EPOCH:
            ...
            logger.info('training conf')
            optimizer, lr_scheduler = build_opt_lr(config, model.module, epoch)
            print('==========double check trainable==========')
            check_trainable(model, logger)  # Print trainable params info
        elif epoch == config.USOT.TRAIN.MEMORY_EPOCH:
            logger.info('training neck+head')
            optimizer, lr_scheduler = build_opt_lr(config, model.module, epoch)
            print('==========double check trainable==========')
            check_trainable(model, logger)  # Print trainable params info
