...
class USOTTracker(object):
    ...
    def init(self, im, target_pos, target_sz, model):
        ...
        yamlPath = os.path.join(absPath, '../../experiments/test/SSL/{0}'.format(yname))
        ...
        x_crop, crop_info = get_subwindow_tracking(im, target_pos, p.instance_size,       # x_crop(255,255,3)
                                                   python2round(s_x), avg_chans, target_sz,
                                                   out_mode='raw', need_bbox=True, vis=False)
        search_bbox = crop_info['template_bbox']    # list:4
        x_crop_unsq = im_to_torch(x_crop.copy())
        x_crop_unsq = x_crop_unsq.unsqueeze(0)     # x_crop_unsq(1,3,255,255)
        search_bbox_pool = self.pool_label_search(p, search_bbox)
        search_bbox_pool = torch.tensor([search_bbox_pool]).float().cuda()   #    tensor(1,4)
        memory_feature = net.extract_memory_feature(ori_x=x_crop_unsq.cuda(), search_bbox=search_bbox_pool)    # (1,256,7,7)
        ...
        x_crop_aug_unsq = x_crop_aug.unsqueeze(0)   # 1,3,255,255
        ...
        memory_feature_aug = memory_feature_aug.cpu().detach()  # 1,256,7,7
    ...
    def update(self, net, x_crops, target_pos, target_sz, window,
        ...
        cls_score, bbox_pred, cls_memory, xf, conf = net.track(x_crops, template_mem=template_mem, score_mem=score_mem)
        ...
        # 没有在线更新
        # cls_score, bbox_pred, cls_memory, xf = net.track(x_crops, template_mem=None, score_mem=None)
        # cls_score = F.sigmoid(cls_score).squeeze().cpu().data.numpy()
        ...
        return target_pos, target_sz, cls_score[r_max, c_max], feature_mem, conf
    ...
    def track(self, state, im):
        ...
        target_pos, target_sz, confidence, feat_mem, conf = self.update(net, x_crop.cuda(), target_pos,
        ...
        # 置信度小于0.4就不把新获得的特征图作为模板了,不再更新往里面加新模板了
        if conf >= 0.4:
